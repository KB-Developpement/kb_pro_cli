...
// updateCheckDone is closed when the background release-fetch goroutine finishes.
// Execute() waits on it (up to 2 s) so the goroutine can write its state file
...
// runUpdateCheck reads the cached update state and prints a one-line notice to
// stderr if a newer version is available. If the cache is stale or missing it
// starts a background goroutine to refresh it.
func runUpdateCheck() {
